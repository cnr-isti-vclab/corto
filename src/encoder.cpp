...
#ifdef FB_CORTO_CHANGES
void Encoder::setZstdCompressionLevel(int compressionLevel) {
...
#ifdef FB_CORTO_CHANGES
bool Encoder::setStrategyAttribute(
    const char* name,
...
#ifdef FB_CORTO_CHANGES
const uint32_t UNINITIALIZED_VALUE = 0xffffffff;
class McFaceSides {
public:
	uint32_t t[3]; //topology: opposite face
	uint32_t i[3]; //index in the opposite face of this face: faces[f.t[k]].t[f.i[k]] = f;
...
/*
This function updates the "faceSides" vector based on "faces" vector
The information updated is:
faceSides[face].t[3]: the opposite face. There are 3 of them, for each edge
faceSides[face].i[3]: the index of "t" vector at the opposite face,
                      which points to the current face
*/
static void buildTopologyOpt(const std::vector<uint32_t>& faces, vector<McFaceSides>& faceSides, uint32_t nvert) {
	// compute buckets size for edges with lower vertex in common
	vector<uint32_t> count(nvert, 0);
	for (int i=0;i< faces.size();i+=3) {
		count[min(faces[i  ], faces[i + 1])]++; // Edge 0-1
		count[min(faces[i+1], faces[i + 2])]++; // Edge 1-2
		count[min(faces[i+2], faces[i    ])]++; // Edge 2-0
	}
	...
	uint32_t facesCount = faces.size() / 3;
	// Create the edges vector
	vector<McEdge> edges(faces.size());
	for (size_t i = 0; i < facesCount; i++) {
		uint32_t base = i * 3;
		uint32_t f0 = faces[base];
		uint32_t f1 = faces[base+1];
		uint32_t f2 = faces[base+2];

		uint32_t v0 = min(f1, f2);
		uint32_t v1 = min(f2, f0);
		uint32_t v2 = min(f0, f1);

		// Construct the edges in place
		McEdge& pos1 = edges[count[v0]++];
		pos1.face = i;
		pos1.side = 0;
		if (f1 < f2) {
			pos1.v0 = f1;
	...
	McEdge previous(UNINITIALIZED_VALUE, UNINITIALIZED_VALUE, UNINITIALIZED_VALUE, UNINITIALIZED_VALUE);

	// Identify opposite edges
	for (const McEdge& edge : edges) {
		if (edge.match(previous)) {
			uint32_t& edge_side_face = faceSides[edge.face].t[edge.side];
			uint32_t& previous_side_face = faceSides[previous.face].t[previous.side];
			if (edge_side_face == 0xffffffff && previous_side_face == 0xffffffff) {
				edge_side_face = previous.face;
				faceSides[edge.face].i[edge.side] = previous.side;
				previous_side_face = edge.face;
				faceSides[previous.face].i[previous.side] = edge.side;
			}
		}
		else
			previous = edge;
...
void Encoder::encodeFacesOpt(int start, int end) {

	// Instead of "MCFace" object containing both faces and the neighboring
	// faces, we use the original faces vector, and create a separate
	// "sides" vector to hold the neighboring faces
	vector<McFaceSides> faceSides(end - start);
	// Initialize "t" and "i" member variables of every object to a default state
	memset(faceSides.data(), UNINITIALIZED_VALUE, sizeof(McFaceSides) * faceSides.size());

	// Populate the neighbouring faces
	buildTopologyOpt(index.faces, faceSides, nvert);
	...
	uint32_t faces_size = faceSides.size();
	vector<bool> visited(faces_size, false);
	unsigned int totfaces = faces_size;
	...
	while (totfaces > 0) {
		if (new_edge == -1 && order >= faceorder.size() && !delayed.size()) {
			...
			while (current != faces_size) {   //find first triangle non visited
				if (!visited[current]) break;
				current++;
			}
			if (current == faces_size) break; //no more faces to encode exiting

			//encode first face: 3 vertices indexes, and add edges
			unsigned int current_edge = front.size();
			//McFace& face = faces[current];
			uint32_t* f = &index.faces[current * 3];
		...
		//opposite face is the triangle we are encoding
		uint32_t opposite_face = faceSides[e.face].t[e.side];
		int opposite_side = faceSides[e.face].i[e.side];

		if (opposite_face == UNINITIALIZED_VALUE || visited[opposite_face]) { //boundary edge or glue
			index.clers.push_back(BOUNDARY);
			continue;
		}

		assert(opposite_face < faces_size);
		//McFace& face = faces[opposite_face];
		uint32_t* f = &index.faces[opposite_face * 3];
		...
		bool close_left = (faceSides[previous_edge.face].t[previous_edge.side] == opposite_face);
		bool close_right = (faceSides[next_edge.face].t[next_edge.side] == opposite_face);
		...
		if (close_left && close_right) {
			...
			if (encoded[opposite] != -1) {
				...
				index.clers.push_back(VERTEX);
				//vertex needed for parallelogram prediction
				uint32_t* f = &index.faces[e.face * 3];
				//int v2 = faces[e.face].f[e.side];
				int v2 = f[e.side];
				prediction[current_vertex] = Quad(opposite, v0, v1, v2);
				encoded[opposite] = current_vertex++;
...
void Encoder::setOptimizationMode(bool optimizeForSpeed)
{
	this->optimizeForSpeed = optimizeForSpeed;
}

#endif // FB_CORTO_CHANGES
